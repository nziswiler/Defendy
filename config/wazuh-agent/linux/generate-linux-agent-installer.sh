...
PACKAGEBEAT_YAML="https://raw.githubusercontent.com/nziswiler/Defendy/main/config/wazuh-agent/linux/packetbeat.yml"
...
if [ -f "/etc/redhat-release" ]; then
    ...
    curl -LO $PACKAGEBEAT_YAML && sudo rpm -vi packetbeat-8.11.1-x86_64.rpm
    ...
    sudo systemctl enable wazuh-agent
    sudo systemctl start wazuh-agent
    curl -LO $PACKAGEBEAT_YAML && sudo dpkg -i packetbeat-8.11.1-amd64.deb
    sudo service packetbeat restart
else
